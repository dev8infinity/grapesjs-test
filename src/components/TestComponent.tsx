...
export function TestComponent(editor: Editor) {


   
    setInterval(() => {
        const tree = editor.getComponents();

        console.log(editor.getHtml()); // For saving or sending to a server  
        console.log(JSON.stringify(tree.toJSON())); // For saving or sending to a server
    ...
    editor.DomComponents.addType('icon-text', {
        model: {
            defaults: {
                ...
                draggable: true,
                droppable: true,
                attributes: { class: 'icon-text', href: '#', target: '_blank' },
                components: [
                    {
                        type: 'image',
                        style: {
